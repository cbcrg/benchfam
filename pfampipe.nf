...
base_path = "$PWD/tutorial/dataset"
...
process Large_scale_MSAs {
    ...
    output: 
        file '*.aln' into aln_out

    script:
    if( method=='mafft')
        """
        mafft --anysymbol --parttree --thread ${params.cpus} --quiet ${dataset} > ${dataset.baseName}_${method}.aln
        """

    else if( method=='clustalo' )
        """
        clustalo --threads ${params.cpus} -i ${dataset} -o ${dataset.baseName}_${method}.aln
        """
...
process splib {
    input: 
       val fam from famNames.split(' ')
	   
    output: 
       file '*.sp_lib' into spLib
	
    """
    t_coffee -lib ${base_path}/$fam/20130714/OUTPUT/sap.lib ${base_path}/$fam/20130714/OUTPUT/mustang.lib ${base_path}/$fam/20130714/OUTPUT/tmalign.lib -output sp_ascii -outfile ${fam}.sp_lib
    """
}
...
process extractedMSA {
    ...
    output: 
    file '*.extracted_msa' into extracted_msa
